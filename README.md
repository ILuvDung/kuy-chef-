...
getgenv()['Check_Setting'] = function(Name)
    if not _G.Dis and not isfolder('KUY cheff Hub') then
        makefolder('KUY cheff Hub')
    end
    if not _G.Dis and not isfile('KUY cheff Hub/'..Name..'.json') then
        writefile('KUY cheff Hub/'..Name..'.json',JsonEncode(_G.Setting_table))
    end
end
getgenv()['Get_Setting'] = function(Name)
    if not _G.Dis and isfolder('KUY cheff Hub') and isfile('KUY cheff Hub/'..Name..'.json') then
        _G.Setting_table = JsonDecode(readfile('KUY cheff Hub/'..Name..'.json'))
...
local win = library:Window("KUY cheff",[[Hub]],[[Version : Premium ]],"9606070311",Enum.KeyCode.RightControl)
